...
export const buttonPressed = asyncHandler(async (req: Request, res: Response) => {
  const { deviceId } = req.params;
  const { targetTeacherId } = req.body;
  const visit = await deviceActionService.handleDoorbellButtonPressedByDeviceId(deviceId, targetTeacherId);
  res.json({ visit });
});
...
export const teacherSelected = asyncHandler(async (req: Request, res: Response) => {
  const { deviceId } = req.params;
  const { teacherId } = req.body;
  await deviceActionService.handleTeacherSelectedByDeviceId(deviceId, teacherId);
